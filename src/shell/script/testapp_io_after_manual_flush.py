...
class TestApp(TestAppBase):
    def main(self):
        ...
        lba_range = self.get_random_lba_range(ssd_config.COMMAND_BUFFER_SIZE)
        expected = {}
        ...
        for i in range(5 * ssd_config.COMMAND_BUFFER_SIZE):
            ...
            if opc == 0:
                ...
                ret = self.flush()
                LOGGER.debug(f"[Command {i + 1}] flush()")
                if ret.err:
                    LOGGER.debug(f'Error is occurred with "flush" ({ret.err}, {ret.val})')
                    return ReturnObject(-1, None)
